...
export class ContactService {
    ...
    public createContact(contact: IContact): void {
        ...
        this._http.post('http://158.160.114.225/api/Issues', formData)
    ...
    public deleteContact(id: string): Subscription {
        return this._http.delete(`http://158.160.114.225/api/Issues/${id}`)
            .pipe(takeUntilDestroyed(this._destroyRef))
            .subscribe(() => {
                this.loadContacts();
            });
    ...
    private fetchContacts(): Observable<IContact[]> {
        return this._http.get<IContact[]>('http://158.160.114.225/api/Issues');
    }
}
